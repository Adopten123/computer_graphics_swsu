...
void draw_polyline(CImg<unsigned char>& img, const polyline& poly, const unsigned char color[3], bool closed = false) {
    ...
    if (closed and poly.size() > 2)
        img.draw_line((int)std::round(poly.back().x), (int)std::round(poly.back().y), (int)std::round(poly[0].x), (int)std::round(poly[0].y), color);
}

void draw_points(CImg<unsigned char>& img, const polyline& pts, const unsigned char color[3], int radius = 3) {
    for (auto& p : pts)
...
polyline build_quadratic_bezier_curve(const polyline& control, int steps) {
    ...
    for (int i = 0; i <= steps; ++i) {
        ...
        curve.push_back(bezier_quad(control[0], control[1], control[2], t));
    }
    return curve;
}
...
polyline build_cubic_bezier_curve(const polyline& control, int steps) {
    ...
    for (int i = 0; i <= steps; ++i) {
        double t = (double)i / steps;
        curve.push_back(bezier_cubic(control[0], control[1],
                                     control[2], control[3], t));
    }
    return curve;
}
